...
function topupBalance(option) {
    if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.openInvoice) {
        const invoiceLink = `https://t.me/invoice?amount=${option.stars}`;
        alert(`Для пополнения баланса на ⭐ ${option.amount} необходимо оплатить ${option.stars} Telegram Stars. В реальной версии здесь открывается оплата.`);
        balance += option.amount;
        updateBalanceDisplay();
    } else {
        alert(`Демо-режим: добавлено ⭐ ${option.amount}!`);
        balance += option.amount;
        updateBalanceDisplay();
    }
    closeTopupPopup();
